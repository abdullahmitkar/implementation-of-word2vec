...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  ===============================================================================
  """
  # Calculating start and end of window

  start_of_window = 0;
  end_of_window = start_of_window + skip_window * 2;

  # For putting values in the label and batches list
  batch_size_ctr = 0;

  while batch_size_ctr < batch_size:
    center_of_window = start_of_window + skip_window;

    # Using center of the window as context_word
    context_word = data[center_of_window]

    num_skips_iterator = num_skips;
    # Using a 'flip' flag to select word from left or right of the context word
    # at a distance of (num_skips - num_skips_iterator) i.e., increasing distance
    # from the context word based on the current iteration
    flip = 1;
    while num_skips_iterator > 0:
      if flip == -1:
        labels[batch_size_ctr] = data[center_of_window + ((num_skips - num_skips_iterator) * flip) - 1];
        ...
        batch[batch_size_ctr] = context_word;
        batch_size_ctr = batch_size_ctr + 1;
      # Look in the opposite direction the next time
      flip = flip * -1;
      num_skips_iterator -= 1;
...
def build_model(sess, graph, loss_model):
  ...
  with graph.as_default():
    ...
    optimizer = tf.train.GradientDescentOptimizer(0.3).minimize(loss, global_step=global_step)
